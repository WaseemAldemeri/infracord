...
/**
 * Computes the difference between a blueprint and the live state of a Discord guild,
 * producing a set of {@link Action}s that describe what needs to change.
 *
 * `Differ` holds no Discord client — it receives a `Guild` object at diff time and
 * is otherwise stateless between calls. The same instance can be used to reconcile
 * multiple guilds against the same blueprint.
 */
export class Differ {
	 ...
	 */
	public async buildGuildState(guild: Guild): Promise<GuildState> {
	...
	/**
	 * Fetches the live guild state and computes the full set of {@link Action}s
	 * needed to bring the guild in line with the blueprint.
	 *
	 * @remarks Channel and category diffing is not yet implemented — only roles are diffed.
	 */
	public async diff(guild: Guild): Promise<DiffResult> {
